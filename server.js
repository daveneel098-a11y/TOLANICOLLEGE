...
app.get('/api/trigger-seed', (req, res) => {
    try {
        console.log("Forcing first-year student data cleanup and re-seeding via API...");
        ...
        const studentsDirectory = path.join(__dirname, 'q', 'students');
        if (!fs.existsSync(studentsDirectory)) {
            throw new Error(`Directory not found: ${studentsDirectory}`);
        }
        ...
        try { db.exec('ROLLBACK;'); } catch (rollErr) {}
